...
class MovingSquare : public Observer, public Drawable
{
private:
    Vector2 mPosition;
    float mSize;
...
public:
    MovingSquare(Vector2 position, float size)
        : mPosition(position), mSize(size), mInitialY(position.y)
    {
    }

    void draw() const override
    {
        DrawRectanglePro({mPosition.x, mPosition.y, mSize, mSize}, {mSize / 2, mSize / 2}, 0.0f, PURPLE);
    }

    void update(float value) override
    {
        mPosition.y = mInitialY - (value * 3.0f);
    }
};
...
int main()
{
    ...
    Slider slider({400, 700}, {500, 20}, {25, 90});

    std::vector<Observer*> observers
    {
        new Circle({100, 500}, 50),
        new Square({400, 500}, 100),
        new CircleColorChange({700, 500}, 50),
        new MovingSquare({400, 420}, 50)
    };
